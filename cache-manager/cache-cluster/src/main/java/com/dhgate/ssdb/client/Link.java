...
/**
 * Add request(String) method on origin java client.
 */
public class Link {
	private Socket sock;
    ...
    public boolean isConnected() {
		return sock != null && sock.isBound() && !sock.isClosed()
			&& sock.isConnected() && !sock.isInputShutdown()
			&& !sock.isOutputShutdown();
    }
    
    public Response request(String cmd) throws Exception {
		return this.request(cmd, new ArrayList<byte[]>());
	}
	...
	public Response request(String cmd, String... params) throws Exception {
		ArrayList<byte[]> list = new ArrayList<byte[]>();
		for (String s : params) {
			list.add(s.getBytes(AbstractRedisDao.DEFAULT_ENCODING));
		}
		return this.request(cmd, list);
	}
	...
	private List<byte[]> recv() throws Exception {
		...
		while (true) {
			...
			int len = is.read(bs);
			// System.out.println("<< " + (new MemoryStream(bs, 0,
			// len)).printable());
	...
	private List<byte[]> parse() {
		...
		while (true) {
			...
			byte[] data = Arrays.copyOfRange(buf, input.data + idx, input.data
					+ idx + len);
